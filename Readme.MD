In a single Python module called *mosa*, this package implements the Multi-Objective Simulated Annealing (MOSA) heuristic to approximate the Pareto front in multi-objective optimization problems. MOSA is a Monte Carlo algorithm and it is based on the analogy with annealing in metallurgy. It has been initially proposed in [*Optimization by Simulated Annealing*, https://doi.org/10.1126/science.220.4598.671]. As a typical Monte Carlo algorithm, MOSA employs random numbers in the process of searching for optimal solutions to a problem. In the current version of the *mosa* module, the acceptance probability of any randomly generated solution is given by:

$P_{accept}=\alpha \prod_{i=1}^m \min  \left ( 1; \exp \left( \frac{\omega_i \Delta E_i}{T}\right) \right )
+(1 - \alpha) \min \left( 1; \max_{i=1,m} \left(  \exp \left( \frac{\omega_i \Delta E_i}{T}\right) \right)\right)$

A thorough discussion about Multi-Objective Simulated Annealing and its algorithm variants can be found in [*Multiobjective Simulated Annealing: Principles and Algorithm Variants*, https://doi.org/10.1155/2019/8134674], including the equation above (see Eq. 4 in the reference).

In the following sections, the basic workflow of a MOSA run is briefly described. Jupyter notebooks in the test directory provide usage examples.
...
```python
population={"Component":df.Component.values.tolist(),"Concentration":None}
```

In the *population* dictionary above, the "Component" key is filled with data obtained from a *pandas* dataframe. If a key in the population contains a finite list of elements, the corresponding key in the solutions can only contain elements taken from this list. Therefore, the search space represented by the key is **discrete**. On the other hand, the value in the "Concentration" key is **None**. This indicates that the corresponding key in the solutions consists of a list that contains one or more floats randomly chosen within a range defined by lower and upper bounds enclosing a **continuous** search space.

Subsequently, the user must implement a Python function that takes as its single argument a trial solution to the problem and returns a list with the resulting objective values:
...
Finally, the code is provided as is. The author makes no guarantee that its results are accurate and is not responsible for any losses caused by the use of the code.
