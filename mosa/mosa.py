...
class Anneal:
    def __init__(self):
        ...
        print("--------------------------------------------------")
        print("      Developed by Dr. Roberto G. A. Veiga")
    ...
    def evolve(self,func):
        ...
        for key in keys:
            ...
            if population[key] is None:
                print("        Continuous sampling space")
                
                if key in self.__xbounds.keys() and \
                    len(self.__xbounds[key])==2:
                    xbounds[key]=self.__xbounds[key].copy()
        ...
        for temp in self.__temp:
            ...
            for j in range(self.__niter):
                ...
                if poptmp[key] is not None:
                    r=uniform(0.0,1.0)
                    
                    while True:                      
                        new=lstep[key]+int(triangular(-xstep[key],0,
                                                      xstep[key]))
                ...
                if xnel[key]==1 or r<=exchangeprob[key]:
                    ...
                    if r<0.5:
                        ...
                        if poptmp[key] is not None and xdistinct[key]:
    ...
    def purgedominated(self,xset={},delduplicated=False):
        '''
        purgedominated(xset,delduplicated) -> returns a subset of the full or 
    ...
    solution_bounds : dictionary, optional
        A Python dictionary where each key corresponds to a key in the solution 
        set and specifies the lower and upper values of solution elements. 
        Default value is {}, which means that [-1.0,1.0] will be the lower and 
        upper bounds of all solutions.
    ...
    def solution_bounds(self,val):
        if isinstance(val,dict):
            for key,value in val.items():
                if isinstance(value,list) and len(value)==2 and \
                    value[1]>value[0]:
                    self.__xbounds[key]=value
                else:
                    raise MOSAError("Second element in key '%s' must be greater than the first one!" 
                                     % key)
        else:
            raise  MOSAError("Solution bounds must be provided in a dictionary!")        
